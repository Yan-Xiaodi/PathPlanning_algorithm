...
class node
{
  public:
    ...
    {
        float dis = sqrt(pow(parent->point.x - point.x, 2) + pow(parent->point.y - point.y, 2));
...
class RRT
{
  public:
    RRT() = default;
    RRT(cv::Point2f start_, cv::Point2f target_, float stride_, float threshold_, int MaxIteration_,
        int rand_precition_, Map* map_)
        : start(start_), target(target_), stride(stride_), threshold(threshold_), iter_cnt(0),
