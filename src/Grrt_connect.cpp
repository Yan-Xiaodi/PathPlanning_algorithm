...
void GRRT_Connect::selfplanning(std::vector<node*>& Tree1, std::vector<node*>& Tree2, int& iter,
                                bool& isfind, bool reverse)
{
    ...
    if (!checkLineCollision(q_new, q_nearest)) {
        ...
        node* n_tmp = Tree2[nr_index2];

        //按growRate倍增的步长,如果q_new和和n_tmp距离<dismin,则只能采用原步长stride生长
        float growStride = stride;
        while (iter++ < MaxIteration) {
            if (sqrt(pow(n_tmp->point.x - q_new.x, 2) + pow(n_tmp->point.y - q_new.y, 2)) <
                std::max(dismin, growStride)) {
                growStride = stride;
            } else {
                growStride *= growRate;
            }
            ...
            node* n_new2 = new node(q_new2, n_tmp);
...
std::map<int, node*> GRRT_Connect::getPath(node* lastNode)
{
    int index = 0;
...
void GRRT_Connect::binaryPurgeNode(std::map<int, node*>& path, int& iter)
{
